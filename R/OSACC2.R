OSACC2 <-
function(Mrklist,CCdata,outputfile,OSACC2.risk=TRUE,sort.cov="ascending",checked.pval=0.05, perm.reps=10,statest="chisq")
{
...
if(OSACC2.risk) permway<-"OSACC2-risk" else permway<-"OSACC2-hom"
...
cleandata<-totaldata[totaldata$trait!=-9999&!is.na(totaldata$trait),]
...
for (ithrk in uniqrk)
{
if(covsort) updatedsubset<-rkmkdata[rkmkdata$ranks >= ithrk,] else
updatedsubset<-rkmkdata[rkmkdata$ranks <= ithrk,]
...
if(covsort)
{ 
selsubset<-rkmkdata[rkmkdata$ranks >= cutrank,]
		...
		for (i in 1:perm.reps) 
			...
			samplenums <-sample(c(1:numind))  
          	 	samplemkdata<-rkmkdata[samplenums,]
           	  	samplegeno<-samplemkdata[,c("allele1","allele2")]
 	  
			samplenums <-sample(c(1:numind))  
          		samplemkdata<-rkmkdata[samplenums,]
	  		samplestatus<-samplemkdata[,"status"]
 	  
			samplenums <-sample(c(1:numind))  
          		samplemkdata<-rkmkdata[samplenums,]
	  		samplerank<-samplemkdata[,c("ranks","trait")]
          		permnewdata<-cbind(samplerank,samplestatus,samplegeno)
	  		names(permnewdata)<-c("ranks","trait","status","allele1","allele2")
           		...
           		for (ithrk in uniqrk) 
           		{ 
             		  if(covsort)  updatedsubset<-permnewdata[permnewdata$ranks >= ithrk,] else 
			  updatedsubset<-permnewdata[permnewdata$ranks <= ithrk,]
              		  alletable<-table(cbind(updatedsubset$status,updatedsubset$status),cbind(updatedsubset$allele1,updatedsubset$allele2))
 			  #if the permuation data cannot form a 2X2 alletableency table, it skip to next          
             		  if(dim(alletable)[1]*dim(alletable)[2]<4) next 
              		  sc<-colSums(alletable)
              		  sr<-rowSums(alletable)
              		  E <- outer(sr, sc, "*")/sum(alletable)
              		  if(any(E<5)) pval<-fisher.test(alletable)$p.value else
			  if(statest=="chisq")
			  {
              		  pval<-chisq.test(alletable,correct = FALSE)$p.value
	...
	if(!(OSACC2.risk))
	{
		for (i in 1:perm.reps) 
			...
			samplenums <-sample(c(1:numind))  
          		samplemkdata<-rkmkdata[samplenums,]
	  		samplerank<-samplemkdata[,c("ranks","trait")]
          		permnewdata<-cbind(samplerank,rkmkdata[,c("status","allele1","allele2")])
	  		names(permnewdata)<-c("ranks","trait","status","allele1","allele2")
